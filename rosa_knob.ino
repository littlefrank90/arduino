...
bool singlePremuto = false; // utilizzato per fare funzionalità di TOGGLE quando si fa click una volta
...
void loop() {  
  ...
  if (value != last) { // New value is different than the last one, that means to encoder was rotated
    if(last<value) // Detecting the direction of rotation
      Keyboard.write(KEY_RIGHT_ARROW); // Replace this line to have a different function when rotating counter-clockwise
      else
      Keyboard.write(KEY_LEFT_ARROW); // Replace this line to have a different function when rotating clockwise
  ...
  if (b != ClickEncoder::Open) { // If the button is unpressed, we'll skip to the end of this if block
    ...
    switch (b) {
      ...
      case ClickEncoder::Clicked: // Button was clicked once
        Serial.println(singlePremuto);
        if (!singlePremuto){
          Serial.println("NON PREMUTO");
          Keyboard.press(KEY_LEFT_SHIFT); // Replace this line to have a different function when clicking button once
          singlePremuto = true;
        }
        else {
      ...
      case ClickEncoder::DoubleClicked:{ // Button was double clicked
         Keyboard.press(KEY_ESC); // Replace this line to have a different function when double-clicking
         Keyboard.releaseAll();
         singlePremuto = false;
      }
      break;      
